...
class RocketTest: XCTestCase {
    ...
    func testRocketsDecoding() throws {
        let jsonDecoder = JSONDecoder()
        let rockets = try? jsonDecoder.decode([Rocket].self, from: RocketTest.rockets)
        XCTAssertNotNil(rockets, "Rockets decoding error")
        XCTAssertNotEqual(rockets!.count, 0)
    }
    ...
    static let rocket = try! JSONDecoder().decode([Rocket].self, from: rockets).first!
}
